...
@allure.description(f"""
        Заполнение формы регистрации значениями
        ...
        подсвечиваются зеленым, поля заполненные
        невалидными значением - красным.
""")
# Основной тест
@pytest.mark.parametrize('index', range(len(data)))
def test_color_comparison(index, color_get):
    with allure.step(f"""Заданный цвет '{data[index][2]}'
                      поля '{data[index][0]}'
                        со значением '{data[index][1]}'
                     записываем в переменную current_color"""):
        current_color = data[index][2]
    with allure.step(f"""Полученный цвет '{color_get[index]}'
                      поля '{data[index][0]}'
                        со значением '{data[index][1]}'
